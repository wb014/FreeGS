...
def render_msplat(viewpoint_camera, pc: GaussianModel, pipe, bg_color: torch.Tensor, scaling_modifier=1.0,
           override_color=None):
    """
    ...
    if override_color is None:
        ...
        sh2rgb = ms.compute_sh(shs, direction)
        feats = torch.clamp_min(sh2rgb + 0.5, 0.0)
    else:
        feats = override_color

    render = ms.rasterization(
        position, scaling, rotation, opacity, feats, intrinsic_params, extrinsic_matrix, width, height, bg_color[0].item()
    )
...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, opt, scaling_modifier = 1.0, override_color = None):
    ...
    if opt.include_feature:
        ...
        language_id = pc.get_language_id
    else:
        language_feature_precomp = torch.zeros((1,), dtype=opacity.dtype, device=opacity.device)
        language_id = torch.zeros((1,), dtype=opacity.dtype, device=opacity.device)
    # Rasterize visible Gaussians to image, obtain their radii (on screen). 
    # start_time = time.time()

    rendered_image, language_feature_image, language_id_mask,  radii = rasterizer(
        means3D = means3D,
        means2D = means2D,
        shs = shs,
    ...
    return {"render": rendered_image,
            ...
            "language_id_mask": language_id_mask,
