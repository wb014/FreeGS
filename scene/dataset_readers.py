...
def fetchPly(path, mode=None):
    plydata = PlyData.read(path)
    vertices = plydata['vertex']
    positions = np.vstack([vertices['x'], vertices['y'], vertices['z']]).T
    colors = np.vstack([vertices['red'], vertices['green'], vertices['blue']]).T / 255.0
    normals = None if mode=='scannet' else np.vstack([vertices['nx'], vertices['ny'], vertices['nz']]).T
...
def readScanNetSceneInfo(path, images, eval, llffhold=8):
    cam_infos = []
    ...
    try:
        pcd = fetchPly(ply_path, mode='scannet')
    except:
        pcd = None
...
def readColmapSceneInfo(path, images, eval, llffhold=8):
    try:
        ...
        cam_extrinsics = read_extrinsics_text(cameras_extrinsic_file, mode='scannet' if 'scene' not in path else None)
