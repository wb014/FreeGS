...
class SimpleDownsampler(torch.nn.Module):
    ...
    def __init__(self, kernel_size, final_size, *args, **kwargs):
        ...
        self.kernel_params = torch.nn.Parameter(torch.ones(kernel_size)) if len(kernel_size)==2 else torch.nn.Parameter(torch.ones(kernel_size, kernel_size))

    def forward(self, imgs, guidance=None):
        b, c, h, w = imgs.shape
        input_imgs = imgs.reshape(b * c, 1, h, w)
        stride = (h - self.kernel_size) // (self.final_size - 1) if h == w else ((h - self.kernel_size[0]) // (self.final_size - 1), (w - self.kernel_size[1]) // (self.final_size - 1))
...
class AttentionDownsampler(torch.nn.Module):
    ...
    def __init__(self, dim, kernel_size, final_size, blur_attn, *args, **kwargs):
        ...
        self.w = torch.nn.Parameter(torch.ones(self.kernel_size).cuda()
                                    + .01 * torch.randn(self.kernel_size).cuda())
        self.b = torch.nn.Parameter(torch.zeros(self.kernel_size).cuda()
                                    + .01 * torch.randn(self.kernel_size).cuda())
    ...
    def forward(self, hr_feats, guidance=None):
        ...
        patches = torch.nn.Unfold(self.kernel_size, stride=stride)(inputs) \
            .reshape(
            (b, self.in_dim, self.kernel_size[0] * self.kernel_size[1], self.final_size, self.final_size * int(w / h))) \
            .permute(0, 3, 4, 2, 1)
